...
public static class Route
{
    ...
    {
        app.MapGet("/library/userbook/v1/", async ([FromQuery] bool wishlist, HttpContext httpContext, CancellationToken cancellationToken, Handler handler) =>
            {
                var userId = httpContext.User.Claims.First(q=> q.Type == ClaimTypes.Name).Value;
                var request = new Request() { UserId = userId, WishList=wishlist };
